...
> **최초 작성일:** 2026-05-18  
> **최종 업데이트:** 2026-06-02 (Task 008 완료 — MVP 배포 완료)  
> **담당자:** JasonPark2020
...
> **구현 전 참고:** `getCategories()`는 `getPublishedPosts()` 캐시를 재사용하므로 별도 API 호출 없음. 카테고리 카드는 기존 스타일링 가이드의 히어로 헤더 패턴(`bg-slate-900`) 활용.
...
| `app/(marketing)/categories/page.tsx` | **미구현** | 전체 카테고리 목록 페이지 (Task 009) |
