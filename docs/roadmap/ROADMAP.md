...
| Task 002 | Task 002: Notion Integration 생성 및 데이터베이스 구성 | 🔲 대기중 | Task 001 | 골격 |
...
- [ ] `.env.example` 파일에 키 이름과 설명 주석 추가 여부 확인 및 보완
- [ ] `lib/env.ts`에 환경 변수 Zod 스키마 정의 (`@t3-oss/env-nextjs` 활용)
...
**Task 002: Notion Integration 생성 및 데이터베이스 구성** | 상태: 🔲 대기중

**구현 항목**
- [ ] [Notion Integrations](https://www.notion.so/my-integrations)에서 Internal Integration 생성
- [ ] Integration Secret 복사 후 `.env.local`의 `NOTION_TOKEN`에 저장
- [ ] Notion에 블로그 데이터베이스 생성 (아래 스키마 적용)
  - `Title` — title 타입
  - `Category` — select 타입 (예시 옵션: AI, 개발, 리뷰)
  - `Tags` — multi_select 타입
  - `Published` — date 타입
  - `Status` — select 타입 (옵션: 초안, 발행됨)
- [ ] 데이터베이스 페이지에서 Integration 연결 (Share > Invite)
- [ ] 데이터베이스 ID 추출 후 `.env.local`의 `NOTION_DATABASE_ID`에 저장
- [ ] `Status = 발행됨`인 테스트 포스트 3개 이상 입력
...
**완료 조건 (Definition of Done)**
- [ ] `.env.local` 실제 토큰 및 데이터베이스 ID 설정 완료
- [ ] 테스트 포스트 3개 이상 입력 완료
- [ ] `getPublishedPosts()` 함수 호출 시 실제 데이터 반환 확인 (콘솔 로그 또는 홈 페이지 렌더링)
...
| Task ID | Task명 | 상태 | 의존성 | 분류 |
|---------|--------|------|--------|------|
| Task 004 | Task 004: Notion 블록 렌더러 구현 | 🔲 대기중 | Task 002 | 공통 |

#### Task 상세

---

**Task 004: Notion 블록 렌더러 구현** | 상태: 🔲 대기중

> **기술 위험:** `@notionhq/client` v5에서 `dataSources` API를 사용하므로 블록 구조가 기존 v2 문서와 다를 수 있음. 구현 전 `node_modules/next/dist/docs/` 및 `@notionhq/client` 타입 정의 파일을 반드시 확인할 것.

**구현 항목**
- [ ] `components/features/NotionBlockRenderer.tsx` 컴포넌트 생성
- [ ] 지원할 블록 타입별 렌더링 구현:
  ...
  - `image` — 이미지 블록 (Next.js `<Image>` 컴포넌트 활용)
  - `quote` — 인용구
  - `divider` — 구분선
  - `toggle` — 토글 (기본 열림 상태)
  - 미지원 블록 — `[지원하지 않는 블록: {type}]` 플레이스홀더 표시
- [ ] `notion-to-md` 라이브러리 활용 여부 검토 (직접 구현 vs 라이브러리)
- [ ] `prose` Tailwind 클래스를 활용한 Typography 스타일 적용
- [ ] 중첩 블록(`has_children = true`) 재귀 렌더링 처리
- [ ] Playwright MCP를 활용한 블록 렌더링 테스트
  - 각 블록 타입이 올바른 HTML 요소로 렌더링되는지 확인
...
| Task ID | Task명 | 상태 | 의존성 | 분류 |
|---------|--------|------|--------|------|
| Task 003 | Task 003: 홈 페이지 Notion 데이터 연동 검증 | 🔲 대기중 | Task 002 | 개별 |
| Task 005 | Task 005: 글 상세 페이지 블록 렌더링 연동 | 🔲 대기중 | Task 004 | 개별 |
| Task 006 | Task 006: 카테고리 페이지 동작 검증 | 🔲 대기중 | Task 003 | 개별 |

#### Task 상세

---

**Task 003: 홈 페이지 Notion 데이터 연동 검증** | 상태: 🔲 대기중

> **현황 메모:** `app/(marketing)/page.tsx`에 홈 페이지 서버 컴포넌트가 구현되어 있음. 환경 변수 미설정 시 목업 데이터로 폴백하는 로직 포함. `PostListClient`, `CategoryFilter`, `PostCard` 컴포넌트도 구현 완료. Task 002 완료 후 실제 Notion 데이터로 동작 검증만 필요.

**구현 항목**
- [ ] `.env.local` 설정 후 홈 페이지에서 실제 Notion 포스트 목록 렌더링 확인
- [ ] 목업 데이터 안내 배너가 사라지고 실제 데이터가 표시되는지 확인
- [ ] 카테고리 필터 탭 클릭 시 필터링 동작 확인
- [ ] 포스트 카드에 제목, 카테고리, 발행일, 태그가 올바르게 표시되는지 확인
- [ ] Notion API Rate Limit (초당 3 요청) 대응: Next.js 캐싱 설정 확인 (`revalidate` 옵션)
...
- [ ] 모바일(375px), 태블릿(768px), 데스크탑(1280px) 레이아웃 정상 표시 확인

---

**Task 005: 글 상세 페이지 블록 렌더링 연동** | 상태: 🔲 대기중

> **현황 메모:** `app/(marketing)/posts/[id]/page.tsx` 159번 줄에 `{/* TODO: Notion 블록 렌더러 구현 후 blocks 데이터 사용 */}` 플레이스홀더 존재. Task 004 완료 후 `NotionBlockRenderer` 컴포넌트를 연동하면 됨.

**구현 항목**
- [ ] `post.blocks` 데이터를 `NotionBlockRenderer` 컴포넌트에 전달하도록 `PostPage` 수정
- [ ] TODO 플레이스홀더 제거 및 실제 블록 렌더링으로 교체
- [ ] 이전 글 / 다음 글 내비게이션 구현
  - `getPublishedPosts()` 조회 결과에서 현재 포스트 위치 기준 인접 포스트 링크 생성
- [ ] 포스트 메타 정보(제목, 발행일, 카테고리, 태그) 표시 확인
- [ ] Playwright MCP를 활용한 E2E 테스트 수행
...
- [ ] 이전/다음 글 내비게이션 구현 완료
- [ ] Playwright E2E 테스트 통과
- [ ] generateMetadata 함수가 실제 포스트 제목으로 메타데이터 생성 확인

---

**Task 006: 카테고리 페이지 동작 검증** | 상태: 🔲 대기중

> **현황 메모:** `app/(marketing)/categories/[category]/page.tsx`가 이미 구현되어 있음. Task 003 완료 후 실제 Notion 데이터로 동작 검증만 필요.

**구현 항목**
- [ ] 실제 Notion 데이터로 카테고리 페이지 렌더링 확인
- [ ] 홈 페이지의 카테고리 배지 클릭 → `/categories/[category]` 이동 동작 확인
- [ ] 글 상세 페이지의 카테고리 배지 클릭 → 카테고리 페이지 이동 동작 확인
- [ ] 한글 카테고리 이름 URL 인코딩/디코딩 정상 동작 확인
- [ ] 해당 카테고리 포스트가 없을 때 빈 상태 메시지 표시 확인
- [ ] Playwright MCP를 활용한 E2E 테스트 수행
...
| Task ID | Task명 | 상태 | 의존성 | 분류 |
|---------|--------|------|--------|------|
| Task 007 | Task 007: 캐싱 전략 적용 (ISR / On-demand Revalidation) | 🔲 대기중 | Task 005, Task 006 | 최적화 |
...
**Task 007: 캐싱 전략 적용 (ISR / On-demand Revalidation)** | 상태: 🔲 대기중

> **기술 위험:** Next.js 16.x의 캐싱 API(fetch `next.revalidate` 옵션 등)가 기존 버전과 다를 수 있음. 구현 전 `node_modules/next/dist/docs/` 내 캐싱 관련 문서를 확인할 것.

**구현 항목**
- [ ] Next.js 16.x 캐싱 메커니즘 확인 (`node_modules/next/dist/docs/` 참조)
- [ ] 홈 페이지(`/`)에 ISR 적용: `revalidate` 시간 설정 (권장: 300초 = 5분)
- [ ] 글 상세 페이지(`/posts/[id]`)에 `generateStaticParams` 적용 고려
- [ ] Notion API 호출 함수(`lib/notion.ts`)에 Next.js 캐시 태그 적용
- [ ] On-demand Revalidation을 위한 Webhook API 라우트 구현 (`app/api/revalidate/route.ts`)
  - Notion Webhook 또는 수동 트리거 방식으로 캐시 갱신 가능하도록 구성
- [ ] Playwright MCP를 활용한 캐싱 동작 검증
  - 페이지 재접속 시 캐시된 데이터가 반환되는지 확인
  - Revalidation 후 갱신된 데이터가 반영되는지 확인
...
**완료 조건 (Definition of Done)**
- [ ] 캐싱 전략 구현 완료
- [ ] Notion API Rate Limit 초과 없음 확인
- [ ] Playwright 테스트 통과
- [ ] 캐시 갱신 동작 검증 완료
...
## 현재 구현 상태 요약 (2026-05-18 기준)

> PRD 분석 시점에서 이미 구현된 항목을 기록합니다.
...
| `app/(marketing)/posts/[id]/page.tsx` | 부분 완료 | 블록 렌더러 플레이스홀더 존재 (Task 005 대기) |
| `app/(marketing)/categories/[category]/page.tsx` | 완료 | 카테고리별 포스트 목록 페이지 구현됨 |
| `components/features/PostCard.tsx` | 완료 | 포스트 카드 컴포넌트 구현됨 |
...
| `components/features/NotionBlockRenderer.tsx` | 미구현 | Task 004에서 구현 예정 |
| `.env.local` | 미완료 | 실제 `NOTION_TOKEN`, `NOTION_DATABASE_ID` 값 입력 필요 |
