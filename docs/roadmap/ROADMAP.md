...
> **최초 작성일:** 2026-05-18  
> **최종 업데이트:** 2026-06-02 (Task 009 완료 — 카테고리 목록 페이지)  
> **담당자:** JasonPark2020
...
**구현 항목**
- [ ] `app/(marketing)/categories/page.tsx` 생성
...
- [ ] 헤더 내비게이션에 "카테고리" 링크 추가 (`components/layout/blog-header.tsx`)
  - 데스크탑: nav 링크 추가
  - 푸터에도 카테고리 링크 추가 (`components/layout/site-footer.tsx`)
- [ ] 빈 카테고리 없을 때 빈 상태 UI 처리
...
**완료 조건 (Definition of Done)**
- [ ] 페이지 구현 완료 (`revalidate = 300` 포함)
- [ ] 헤더/푸터 내비게이션 링크 추가 완료
- [ ] Playwright 테스트 통과
...
> **구현 전 참고:** `PostListClient`는 이미 `"use client"` 컴포넌트. `usehooks-ts`의 `useDebounceValue`(v3+) 또는 `useDebouncedCallback` 활용. 검색은 서버 재호출 없이 클라이언트 메모리 내 필터링만 수행.

**구현 항목**
- [ ] `PostListClient` 컴포넌트에 검색 입력 필드 추가
  - shadcn/ui `Input` 컴포넌트 활용
  - placeholder: `"제목 또는 태그로 검색..."`
  - 검색 아이콘(`lucide-react Search`) 좌측 배치
- [ ] 검색 상태 관리 (`useState<string>`)
- [ ] `useDebounceValue(searchQuery, 300)` 적용 — 입력 후 300ms 후 필터 적용
- [ ] 필터 로직 통합: 카테고리 필터 AND 검색어 필터 동시 적용
  ```ts
  // 필터 우선순위: 카테고리 → 검색어 순으로 축소
...
- [ ] 검색 결과 없을 때 빈 상태 메시지 표시
  - "검색 결과가 없습니다" + 검색어 초기화 버튼
- [ ] Playwright MCP를 활용한 테스트
  - 검색어 입력 시 제목/태그 기반 필터링 동작 확인
  - 카테고리 필터 + 검색어 동시 적용 동작 확인
  - 디바운스 동작 확인 (입력 중 즉시 반응 없음)
  - 빈 상태 메시지 표시 확인
...
**완료 조건 (Definition of Done)**
- [ ] 검색 기능 구현 완료
- [ ] 디바운스 적용 확인 (`useDebounceValue` 또는 `useDebouncedCallback`)
- [ ] Playwright 테스트 통과
- [ ] TypeScript 타입 오류 없음 (`npm run build` 통과)
...
| `components/features/PostListClient.tsx` | 완료 | 카테고리 필터 + 포스트 목록. 검색 기능 미구현 (Task 010) |
