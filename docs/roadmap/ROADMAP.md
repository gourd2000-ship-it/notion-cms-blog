...
> **최초 작성일:** 2026-05-18  
> **최종 업데이트:** 2026-06-02 (Task 010 완료 — 클라이언트 사이드 검색, Phase 5 전체 완료)  
> **담당자:** JasonPark2020
...
| Task 008 | Task 008: Vercel 배포 및 프로덕션 환경 구성 | ✅ 완료 | Task 007 | 최적화 |
...
**Task 008: Vercel 배포 및 프로덕션 환경 구성** | 상태: ✅ 완료

> **현황 메모 (2026-06-02 완료):** 프로덕션 URL: **https://notion-cms-blog-roan.vercel.app/**  
> GitHub(`notion-cms-blog`) ↔ Vercel 연동 완료. `NOTION_TOKEN`, `NOTION_DATABASE_ID`, `REVALIDATE_SECRET` 환경 변수 설정 완료. Playwright E2E 프로덕션 테스트 통과.  
> **테스트 결과:** 홈(포스트 3개·카테고리 필터) ✅ / 카테고리 페이지(한글 URL) ✅ / 글 상세(본문 렌더링·이전다음 내비) ✅ / 404 처리 ✅ / 반응형 375·768·1280 ✅

**구현 항목**
- [x] GitHub 저장소와 Vercel 연동 (Vercel Dashboard > Import Project)
- [x] Vercel 환경 변수 설정
  - `NOTION_TOKEN` — Production, Preview, Development 환경 모두 설정
  - `NOTION_DATABASE_ID` — 동일하게 설정
  - `REVALIDATE_SECRET` — Production 환경 설정
- [x] 프로덕션 배포 실행 및 빌드 오류 확인
- [x] 배포된 URL에서 홈 페이지, 글 상세, 카테고리 페이지 동작 확인
- [ ] Vercel Analytics 또는 Speed Insights 활성화 ← 선택 사항, 필요 시 추가
- [x] Playwright MCP를 활용한 프로덕션 URL E2E 테스트
  - 홈 페이지 포스트 목록 렌더링 ✅
  - 카테고리 필터 (All → #기록: 3→1개) ✅
  - 글 상세 페이지 본문 렌더링 + 이전/다음 내비게이션 ✅
  - 카테고리 페이지 한글 URL 인코딩 ✅
  - 존재하지 않는 포스트 → 404 처리 ✅
  - 반응형 3뷰포트(375/768/1280) 레이아웃 정상 ✅
...
- [x] Vercel 프로덕션 배포 완료
- [x] 환경 변수 설정 완료
- [x] 주요 페이지 E2E 테스트 통과 (프로덕션 URL 기준)
- [x] 빌드 로그에 오류 없음 확인
...
| Task ID | Task명 | 상태 | 의존성 | 분류 |
|---------|--------|------|--------|------|
| Task 009 | Task 009: 카테고리 목록 페이지 구현 | ✅ 완료 | Task 006 | P1 확장 |
| Task 010 | Task 010: 클라이언트 사이드 검색 기능 구현 | ✅ 완료 | Task 003 | P1 확장 |

#### Task 상세

---

**Task 009: 카테고리 목록 페이지 구현** | 상태: ✅ 완료

> **현황 메모 (2026-06-02 완료):** `app/(marketing)/categories/page.tsx` 신규 구현. `getCategories()` 호출, 카드 그리드, ISR(revalidate=300), 빈 상태 UI. `config/site.ts` mainNav에 카테고리 링크 추가 → BlogHeader/SiteFooter 자동 반영. Playwright E2E 검증 완료(카드 렌더링 ✅, 카드 클릭 → /categories/[category] ✅, 헤더 내비 링크 ✅).

**구현 항목**
- [x] `app/(marketing)/categories/page.tsx` 생성
  - `getCategories()` 호출 → `{ name, count }` 목록 렌더링
  - `export const revalidate = 300` 적용 (ISR 일관성 유지)
- [x] 카테고리 카드 UI 구현
  - 카테고리명, 포스트 수 (`N개의 포스트`) 표시
  - 클릭 시 `/categories/[category]` 이동
  - 카드 호버: `hover:shadow-xl hover:-translate-y-1 transition-all duration-300`
- [x] 헤더 내비게이션에 "카테고리" 링크 추가 (`config/site.ts` mainNav)
  - BlogHeader/SiteFooter 자동 반영 (siteConfig 패턴)
- [x] 빈 카테고리 없을 때 빈 상태 UI 처리
- [x] Playwright MCP를 활용한 테스트
  - `/categories` 렌더링 확인 (카테고리 목록 + 포스트 수)
  - 카테고리 카드 클릭 → `/categories/[category]` 이동 확인
  - 헤더 내비게이션 링크 동작 확인

**수락 기준 (Acceptance Criteria)**
- `/categories` 접속 시 카테고리 목록과 각 카테고리의 포스트 수가 표시됨
- 카테고리 클릭 시 해당 카테고리 포스트 페이지로 이동함
- 헤더 내비게이션에서 카테고리 페이지 접근 가능

**완료 조건 (Definition of Done)**
- [x] 페이지 구현 완료 (`revalidate = 300` 포함)
- [x] 헤더/푸터 내비게이션 링크 추가 완료
- [x] Playwright 테스트 통과
- [x] TypeScript 타입 오류 없음 (`npm run build` 통과)

---

**Task 010: 클라이언트 사이드 검색 기능 구현** | 상태: ✅ 완료

> **현황 메모 (2026-06-02 완료):** `PostListClient.tsx` 수정. 표시용 `inputValue(useState)` + 필터링용 `debouncedQuery(useDebounceValue, 300ms)` 분리. 카테고리 AND 검색어 필터 결합. 검색어 초기화 X버튼, 필터 전체 초기화 버튼. Playwright 검증 완료(제목 검색 ✅, 태그 검색 ✅, AND 조건 ✅, 빈 상태 ✅, 필터 초기화 ✅).

**구현 항목**
- [x] `PostListClient` 컴포넌트에 검색 입력 필드 추가
  - shadcn/ui `Input` 컴포넌트 활용
  - placeholder: `"제목 또는 태그로 검색..."`
  - 검색 아이콘(`lucide-react Search`) 좌측 배치
  - X 버튼(`lucide-react X`) 우측 배치 — 검색어 있을 때만 표시
- [x] 검색 상태 관리: `inputValue`(즉시 표시) + `debouncedQuery`(필터 적용) 분리
- [x] `useDebounceValue('', 300)` 적용 — 입력 후 300ms 후 필터 적용
- [x] 필터 로직 통합: 카테고리 필터 AND 검색어 필터 동시 적용 (대소문자 무시)
- [x] 검색 결과 없을 때 빈 상태 메시지 + "필터 초기화" 버튼 표시
- [x] Playwright MCP를 활용한 테스트
  - 검색어 "notion" 입력 → 제목 기반 1개 필터링 ✅
  - 검색어 "시작" 입력 → 태그 기반 1개 필터링 ✅
  - 카테고리 `#기록` + 검색어 "테스트" → AND 조건 0개 + 빈 상태 ✅
  - "필터 초기화" 클릭 → 전체 3개 복원 ✅
  - X버튼 클릭 → 검색어만 초기화 ✅
...
- 300ms 디바운스로 입력 중 불필요한 재렌더링 없음

**완료 조건 (Definition of Done)**
- [x] 검색 기능 구현 완료
- [x] 디바운스 적용 확인 (`useDebounceValue` v3, TTL 300ms)
- [x] Playwright 테스트 통과
- [x] TypeScript 타입 오류 없음 (`npm run build` 통과)
...
| `lib/notion.ts` | 완료 | `getPublishedPosts`, `getPostById`, `getPostsByCategory`, `getCategories` + `unstable_cache` 래핑 완료 |
| `lib/types.ts` | 완료 | `Post`, `PostDetail`, `NotionBlock`, `Category` 타입 정의됨 |
| `lib/mock-data.ts` | 완료 | 목업 데이터 정의됨 |
| `lib/env.ts` | 완료 | `NOTION_TOKEN`, `NOTION_DATABASE_ID`, `REVALIDATE_SECRET` Zod 스키마 |
| `app/(marketing)/page.tsx` | 완료 | 홈 페이지 서버 컴포넌트. `revalidate = 300` 적용 |
| `app/(marketing)/posts/[id]/page.tsx` | 완료 | `NotionBlockRenderer` + `PostNavigation` + `revalidate = 300` |
| `app/(marketing)/categories/[category]/page.tsx` | 완료 | 카테고리별 포스트 목록. `revalidate = 300`. Playwright 검증 완료 |
| `app/api/revalidate/route.ts` | 완료 | On-demand Revalidation API (`secret` 인증 + `revalidateTag`/`revalidatePath`) |
| `components/features/PostCard.tsx` | 완료 | Stretched Link 패턴: 카테고리 배지 독립 링크(`/categories/`), 제목 링크(`/posts/`) |
| `components/features/PostListClient.tsx` | 완료 | 검색(300ms 디바운스) + 카테고리 필터 AND 결합. 빈 상태 + 필터 초기화 |
| `components/features/CategoryFilter.tsx` | 완료 | 카테고리 필터 탭 컴포넌트 |
| `components/features/NotionBlockRenderer.tsx` | 완료 | paragraph/heading/목록/code/image/quote/divider/toggle/인라인 서식. 중첩 블록 재귀 미지원 |
| `app/(marketing)/categories/page.tsx` | 완료 | 전체 카테고리 목록. 카드 그리드, `revalidate=300`. Playwright 검증 완료 |
| `config/site.ts` | 완료 | mainNav에 카테고리 링크 추가. BlogHeader/SiteFooter 자동 반영 |
| `.env.local` | 완료 | `NOTION_TOKEN`, `NOTION_DATABASE_ID`, `REVALIDATE_SECRET` 설정 완료 |
| `docs/guides/styling_guide.md` | 완료 | Tailwind CSS v4 기반 디자인 시스템 가이드 |
...
| 2026-06-02 | v1.6 | Task 008 완료. Vercel 프로덕션 배포(https://notion-cms-blog-roan.vercel.app/) 확인. Playwright 프로덕션 E2E 테스트 통과(홈·카테고리 필터·글 상세·404·반응형 375/768/1280). MVP 전체 완료. Phase 5 Task 009/010 구현 가이드 구체화(코드 스니펫·디바운스 패턴 추가). 현재 구현 상태 요약 동기화. | JasonPark2020 |
| 2026-06-02 | v1.7 | Task 009 완료. `app/(marketing)/categories/page.tsx` 신규 구현(카드 그리드·빈 상태·ISR). `config/site.ts` mainNav에 카테고리 링크 추가. Playwright E2E 검증(카드 렌더링·클릭 내비·헤더 링크). | JasonPark2020 |
| 2026-06-02 | v1.8 | Task 010 완료. `PostListClient.tsx`에 검색 입력(shadcn Input + Search 아이콘) 추가. `useDebounceValue(300ms)` + 카테고리 AND 검색 필터. X버튼·필터 초기화 버튼. Playwright E2E 검증(제목·태그 검색, AND 조건, 빈 상태, 초기화). Phase 5 전체 완료. | JasonPark2020 |
